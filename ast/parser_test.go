...
func iterateFastJson(node *fastjson.Value, b *testing.B) {
	switch node.Type() {
...
func BenchmarkParser_FastJson(b *testing.B) {
	ast, _ := fastjson.Parse(_TwitterJson)
	iterateFastJson(ast, b)
...
func BenchmarkParser_Parallel_FastJson(b *testing.B) {
	r, _ := fastjson.Parse(_TwitterJson)
	iterateFastJson(r, b)
	b.SetBytes(int64(len(_TwitterJson)))
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
...
func BenchmarkParseSeven_FastJson(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		exists(node)
		node = ast.GetArray("statuses")[3].Get("user", "entities", "url", "urls")
		exists(node)
		node = ast.GetArray("statuses")[3].Get("user", "entities", "url")
		exists(node)
		node = ast.GetArray("statuses")[3].Get("user", "created_at")
		exists(node)
		node = ast.GetArray("statuses")[3].Get("user", "name")
		exists(node)
		node = ast.GetArray("statuses")[3].Get("text")
		exists(node)
	}
}
